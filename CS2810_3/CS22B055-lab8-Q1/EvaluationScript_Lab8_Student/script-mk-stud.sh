...
# TC 1 to 5 - Dijkstra; TC 6 to 10 - Floyd-Warshall
TIME_LMT=25 # time limit in seconds
...
destArray=(6 99 199 399 999 6 99 199 399 999)
...
do
    ...
    do
        ...
        do
            ...
            echo -e "${Bold}testcase $j :${NC} $alg 0 $dest"
            if [ $j -gt 5 ]
            then
                timeout -k 5 $TIME_LMT "./${target}" "--dest" $dest "--src" 0 "--alg" $alg "--out" $output "--in" $input
            else
                timeout -k 5 $TIME_LMT "./${target}" "--in" $input "--out" $output "--alg" $alg "--src" 0 "--dest" $dest
            fi
